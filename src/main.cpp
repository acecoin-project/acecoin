...
#if defined(NDEBUG)
# error "Acecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x359e98fbfcb575cd48432157bfb5254fb30db7d6ed3a393b7435a44d483b3735");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Acecoin: starting difficulty is 1 / 2^12
...
{
    // Acecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight == 1)  
    {
		nSubsidy = 288000000 * COIN;
	}

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Acecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Acecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Acecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Acecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x359e98fbfcb575cd48432157bfb5254fb30db7d6ed3a393b7435a44d483b3735");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "24/Dec/2017 Dick Enberg, Veteran Sportscaster, Is Dead at 82";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d45d2c22d22705426224f524f629789d338236947a7a4a9ff1c7e51278620f40c9ac9ec196b7f7438c82d7aa1e547137a825ce05d6071f65e260849e0872496a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1514055858;
        ...
        assert(block.hashMerkleRoot == uint256("0x3ae3c88058d7d5d19837e9a36b077764eea74d4fe3ad0210ff4d8f841561be2c"));
...
unsigned char pchMessageStart[4] = { 0xbd, 0xd1, 0xd2, 0xc3 }; // Acecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AcecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AcecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AcecoinMiner(CWallet *pwallet)
{
    printf("AcecoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running AcecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AcecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AcecoinMiner, pwallet));
}
